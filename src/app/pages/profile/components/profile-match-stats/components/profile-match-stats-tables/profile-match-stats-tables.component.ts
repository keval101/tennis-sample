...
export class ProfileMatchStatsTablesComponent implements OnInit, AfterViewInit {
  ...
  public selectedGroup: 'hard' | 'clay' | 'grass' | 'indoor' | 'all' = 'hard';
    ...
    this.activatedRoute.params.subscribe((params) => {
      ...
      this.profileService
        ...
        .subscribe((filters) => {
          ...
          this.yearFilters = filters.years.map((year) => ({
            name: year,
            value: year,
          }));
          ...
          this.filtersFormGroup.valueChanges.subscribe((changes) => {
            this.fetchProfileMatchStatsOnTab(
              this.active.name,
              this.getFormControl('year').value
            );
          });
        });
    });
  ...
  public changeActiveGroup(active: TabActiveInterface) {
    ...
    this.selectedGroup = active.active as
      ...
      | 'all';
    let templates: any;
    const activeName = active.active !== 'all' ? active.name : '';
    this.fetchProfileMatchStatsOnTab(activeName);
  ...
  private fetchProfileMatchStatsOnTab(
    activeTab: string,
    year?: string | number
  ) {
    this.subs.unsubscribe();
    ...
    this.subs.add(
      this.profileService
        .getMatchStats(this.profileName, {
          court: activeTab,
          level: '',
          round: '',
          year: year ?? this.getFormControl('year').value,
        })
        .subscribe((matchStats) => {
          this.serveStats = this.parseServiceStats(matchStats.serviceStats);
          this.returnStats = this.parseReturnStats(matchStats.returnStats);
